...
# Dashboard layout
st.title("Canada Power Generation Insights (Goal Programming Model)", class_="title")

# Canada-specific narrative
st.markdown("This dashboard provides an in-depth analysis of power generation in Canada, focusing on emissions, costs, and technology trends. Explore the data and gain insights into Canada's journey towards a sustainable and net-zero emissions future. The results presented here are based on a goal programming model.")

# Create tabs for different sections
tab1, tab2, tab3 = st.tabs(["Generation Overview", "Emissions Analysis", "Cost Analysis"])

with tab1:
    st.header("Power Generation Overview", class_="header")
    ...
    with chart_container:
        st.markdown("### Generation by Source", class_="subheader")
        fig1 = px.bar(filtered_df, x='Year', y='Generation', color='Technology', barmode='group')
        st.plotly_chart(fig1, height=400)  # Adjust height as needed
    with table_container:
        st.subheader("Data Table for Generation Overview", class_="subheader")
        st.dataframe(filtered_df, height=400)  # Adjust height as needed

    st.subheader("Interactive Line Chart for Energy Generation", class_="subheader")
...
with tab2:
    st.header("Emissions Overview", class_="header")
    ...
    st.markdown("### Emissions by Source", class_="subheader")
    fig2 = px.bar(filtered_df, x='Year', y='Emissions', color='Technology', barmode='group')
    st.plotly_chart(fig2)
    
    st.subheader("Emissions Over Time", class_="subheader")
...
with tab3:
    st.header("Cost Analysis", class_="header")
    ...
    st.subheader("Bar Chart for Cost Breakdown", class_="subheader")
